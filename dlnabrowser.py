...
class DLNABrowser:
    ...
    def discover_servers(timeout=3):
        ...
        try:
            ...
            print(f"[!] Failed to send SSDP request: {e}")
            return []
        ...
        try:
            ...
            print(f"[!] Error during discovery capture: {e}")
        finally:
            sock.close()
    ...
    def select_server(self):
        """Discovers servers and lets user pick an active ContentDirectory server."""
        ...
        if not urls:
            ...
            self.desc_url = fallback if fallback else self.DEFAULT_DESC_URL
        elif len(urls) == 1:
            self.desc_url = urls[0]
            print(f"[+] Automatically selected only available server: {self.desc_url}")
        else:
            ...
            try:
                ...
                self.desc_url = urls[choice - 1]
            except (ValueError, IndexError):
                self.desc_url = urls[0]
                print(f"[!] Invalid selection. Defaulting to first server: {self.desc_url}")
    ...
    def _fetch_control_url(self):
        ...
        try:
            ...
            print("[-] Found server, but it doesn't expose a ContentDirectory control interface.")
            return False
        except Exception as e:
            print(f"[!] Failed parsing server description: {e}")
            return False
    ...
    def display_folder(self, menu_items):
        """Prints directory/file menu items cleanly to the terminal screen."""
    ...
    def enqueue_item_by_index(self, menu_items, index, play_queue):
        """Validates index values and pushes targeted file nodes to the PlayQueue."""
    ...
    def handle_batch_enqueue(self, menu_items, raw_tokens, play_queue):
        """Parses batch multi-selections and range expressions. If tokens are empty, targets all folder tracks."""
        target_indices = []
        
        # If no arguments are passed, create a list of all 1-based index numbers in the current folder
        if not raw_tokens:
    ...
    def handle_navigation(self, menu_items, user_choice):
        """Evaluates folder selection or checks history back-tracking conditions."""
        try:
            idx = int(user_choice)
    ...
    def start_ui(self, play_queue):
        """Active interactive browse selection interface."""
        while True:
            ...
            # Handle either a loose single 'q' or a spaced 'q 1 2 3' command string
